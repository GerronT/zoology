  ...
  <div class="max-w-xl mx-auto p-6 bg-gray-100 rounded-lg shadow-md my-6">
    ...
    <form @submit.prevent="submitAnimal" class="animal-form">
      ...
      <div v-for="(group, index) in form.groupings" :key="index" class="bg-white p-4 mb-6 border-l-4 border-green-500 rounded-md">
        <!-- Preselect Existing Group -->
        <div class="mb-6">
          <label for="child-group" class="block font-semibold mb-1">Select from existing {{index == 0 ? "group" : "subgroup"}}</label>
          <vue-select :v-model="group.id" :options="filteredSearchGroups(index)" @search="onSearchGroup" :get-option-label="getGroupLabel" :disabled="!isGroupEditable(index)"  placeholder="Search Group..." class="child-group-select" @update:modelValue="(val) => updateGroupId(group, val)">
            <template #no-options>
              <span v-if="index == 0">
        ...
        <div v-if="!group?.id">
          ...
          <select v-model="group.classification_id" :disabled="!isGroupEditable(index)" class="w-full p-2 border border-gray-300 rounded-md mb-4">
            ...
            <option v-for="c in filteredClassifications(index)" :key="c.id" :value="c.id">
              {{ c.name }}
            </option>
          </select>
          ...
          <select v-model="group.level_id" :disabled="!isGroupEditable(index) || !group.classification_id" class="w-full p-2 border border-gray-300 rounded-md mb-4">
            ...
            <option v-for="l in filteredLevels(index, group.classification_id)" :key="l.id" :value="l.id">
              {{ l.name }}
            </option>
          </select>
...
export default {
  ...
  setup(props) {
    ...
    const fetchGroupValues = (group) => {
      ...
      return group.id ? preselectedGroups.value.find(g => g.id === group.id) : group;
    }
    ...
    const addGroup = () => {
      form.groupings.push({id: null, name: '', classification_id: null, level_id: 5, description: ''});
    };
    ...
    const onSearchGroup = debounce(async (query) => {
      groupSearchQuery.value = query;

      if (!query.trim()) {
        mainGroupOptions.value = [];
        return;
      }
      ...
      try {
        ...
        mainGroupOptions.value = response.data;
      } catch (error) {
    ...
    const filteredSearchGroups = (index) => {
      return index === 0 ? mainGroupOptions.value : childGroupOptions.value[index];
    };
    ...
    const filteredLevels = (index, classificationId) => {
      ...
      if (prevGroup && prevGroup.classification_id == classificationId) {
        return props.levels.filter(l => {
          return levelRanks.value.get(l.id) > levelRanks.value.get(prevGroup.level_id);
        });
      }
    ...
    const isAddSubgroupEnabled = computed(() => {
      ...
      if (actGroup) {
        const requiredFieldsAreFilled = actGroup.classification_id && actGroup.level_id && actGroup.name;
